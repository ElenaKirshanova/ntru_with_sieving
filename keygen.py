...
"""
  ...
  How to:
    ...
    generate a seed via newSeed() and then invoke the getKey() or getLattice() method.
  ...
  Example for NTRU-HRSS:
    ...
    B = HRSSgenerator.getLattice(seed)
  ...
  Example for NTRU-HPS:
    ...
    B = HPSgenerator.getLattice(seed)
...
class NTRUKeyGenerator:
  ...
  """
  def __init__(self, HRSS, n, pseudoinverse=False, q=0, seed=None, h=None):
    self.HRSS = HRSS
    self.n = n
    self.q = q
  ...
  def getLatticeRandom(self):
    """
    Generate the a random q-ary lattice
    """
    n = self.n
    q = self.q

    random = SystemRandom()
    h = [ ( random.randrange(q) ) for i in range(n) ]
  ...
  def getLatticePhi(self,seed):
    """
    Generate the Phi lattice as per Sec. 5.2
    """
    n = self.n
    q = self.q
    phi_n = self.phi_n
    ...
    if self.HRSS:
      h_phi = self.polyToSympy( self.Sq_inverse([-1,1]) ) * self.polyToSympy( h_phi )
      h_phi = self.sympyToPoly( h_phi )

    h_phi = self.Sq_( h_phi )

    if self.pseudoinverse:
      h_phi = list((self.pseudo_inverse(h)))

    if len(h_phi) < dim:
      h_phi = h_phi + [0]*(dim-len(h_phi))

    B = [ [0]*2*dim for i in range(2*dim) ]
    ...
    for i in range(dim):
      for j in range(dim):
        if i==0:
          ...
          B[dim+i][j] = B[dim+i-1][j-1]

    for i in range(dim):
      for j in range(dim):
        if i!=0:
          B[dim+i][j] -= h_phi[n-1-i]
        B[dim+i][j] = int(B[dim+i][j])


    return B

  def getLatticePhiOne(self, seed):
    """
    Appends [1^(n-1)|0^(n-1)], [0^(n-1)|1^(n-1)] to the Phi lattice
    Does not gain in performance relative to other lattices
    """
    B = self.getLatticePhi(seed)
    n = self.n

    B.append( (n-1)*[1] + (n-1)*[0] )
    B.append( (n-1)*[0] + (n-1)*[1] )

    return B

  def pseudo_inverse(self,h):
    """
    generate the pseudoinverse of h as in Lemma 3 from
    https://csrc.nist.gov/CSRC/media/Events/third-pqc-standardization-conference/documents/accepted-papers/nguyen-boosting-hybridboost-pqc2021.pdf
    For experiments with hybrid attacks
    """
    q = self.q
    hsym = self.polyToSympy(h)

    print('computing gcdex for pseudoinverse of h might take some time...')

    res = self.sympyToPoly(gcdex(hsym, self.polyToSympy(self.phi_n))[0])
    nres = len(list(res))
    resmodq = [0]*nres
    for i in range(nres):
      try:
        nom, denom = fraction(res[i])
        resmodq[i]= (nom*pow(denom, -1, q))%q
      except:
        print(res[i])
        RaiseValueError('inverse mod q does not exist')
    return self.sympyToPoly(resmodq)[0]
